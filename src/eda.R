...
seven= as.numeric(cor(seven_videos_100[,1], seven_videos_100[,2]))
five= cor(five_videos_100[,1], five_videos_100[,2])
four= cor(four_videos_100[,1], four_videos_100[,2])
three=cor(three_videos_100[,1], three_videos_100[,2])
correlations= as.data.frame(c(three, four, five, seven))
colnames(correlations)= c("Correlation between duration and % people watching full video")
...
graph2= ggplot(data= bind_rows(view_percentages_total_duration)) + 
  geom_line(data= view_percentages_100_duration, aes(x=video_duration, y= viewed_onehundred_percent, col="g. 100% of the video watched"), alpha= 1) +
  geom_line(data= view_percentages_95_duration, aes(x=video_duration, y= viewed_ninetyfive_percent, col="f. 95% of the video watched"), alpha= 1) + 
  geom_line(data= view_percentages_75_duration, aes(x=video_duration, y= viewed_seventyfive_percent, col="e. 75% of the video watched"), alpha= 1) +
  geom_line(data= view_percentages_50_duration, aes(x=video_duration, y= viewed_fifty_percent, col="d. 50% of the video watched"), alpha= 1) +
  geom_line(data= view_percentages_25_duration, aes(x=video_duration, y= viewed_twentyfive_percent, col="c. 25% of the video watched"), alpha= 1) +
  geom_line(data= view_percentages_10_duration, aes(x=video_duration, y= viewed_ten_percent, col="b. 10% of the video watched"), alpha= 1) +
  geom_line(data= view_percentages_5_duration, aes(x=video_duration, y= viewed_five_percent, col="a. 5% of the video watched"), alpha= 1) + 
...
graph4= ggplot(data= duration_differences, aes(x=duration_differences[,1], y= viewed_onehundred_percent)) + geom_line() +
...
graph5= ggplot(data= video_stats, aes(x=X1, y= X2)) + geom_line(aes(colour= Video)) +labs(x= "Duration (seconds)", y="Percentage of people watching video")
